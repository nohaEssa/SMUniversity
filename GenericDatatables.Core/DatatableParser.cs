        ...
        where TEntity : class
    {
        ...
        public DatatableReply Parse(DatatableParam param)
        {
            try
            {
                ...
                reply.Echo = Convert.ToInt32(param.Echo);
                reply.Columns = string.Join(",", _properties.Select(p => p.ColumnHeader));
                reply.TotalRecords = totalRecords;
                reply.TotalDisplayRecords = displayRecords;
                reply.Data = projector.Project(_sessionObject).ToArray();
                //var reply = new DatatableReply
                //{
