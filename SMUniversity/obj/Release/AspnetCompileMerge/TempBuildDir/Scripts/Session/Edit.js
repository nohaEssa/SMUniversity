...
$(document).ready(function () {
    ...
    var fromTimeArray = $('#hdn_SessionFromTimes').val().split(',');
    var toTimeArray = $('#hdn_SessionToTimes').val().split(',');
    for (var i = 0; i < fromTimeArray.length; i++) {
        var x = '<br /><div class="col-md-10">' +
            ...
            '<input id="cl_first1" class="form-control clockface_1 dateTimePickerCustom1" type="text" name="CourseFromTime" value="' + fromTimeArray[i]+'" required /><br />' +
            '</div>' +
            ...
            '<input id="cl_second1" class="form-control clockface_1 dateTimePickerCustom1" type="text" name="CourseToTime" value="' + toTimeArray[i] +'" required /><br />' +
            '</div>' +
            ' <div class="col-md-4" id="Myremve">' +
            ' <button style="margin-top: 36px;" type="button" class="btn btn red" onclick="Remove_Row(1)" id="rem1" >حذف <i class="fa fa-times"></i></button><input type="hidden" id="hid1" value="1"  /></div><br />' +
            '</div> ' +
            '</div>' +
            '</div>' +
            '</div>';
        ...
        if (count == 0) {
            ...
            $("#Myfirst_clok").append('<input class="form-control clockface_1 dateTimePickerCustom1" type="text" id="cl_first' + document.getElementById("count").value + '" name="CourseFromTime" value="' + fromTimeArray[i] +'" required /><br />');
            ...
            $("#Mysecond_clok").append('<input class="form-control clockface_1 dateTimePickerCustom1" type="text" id="cl_second' + document.getElementById("count").value + '" name="CourseToTime" value="' + toTimeArray[i] +'" required /><br />');
...
function getCurrentHalls() {
    $.ajax({
        type: "POST",
        url: "/Session/GetHalls",
        cache: false,
        contentType: "application/json; charset=utf-8",
        data: "{FromTime:'" + $('#hdn_FromDate').val() +
            "', ToTime:'" + $('#hdn_ToDate').val() +
