    ...
    $.ajax({
        ...
        success: function (response) {
            if (response !== "ERROR") {
                ...
                if ($('input[type=radio][name=GeneralSession]').val() == "true") {
                    //$("#SubjectID").append(new Option("ماده اخري", "-1"));
                    //$(".divNewSubjectName").show();
                }
            }
        },
...
$(document).ready(function () {
    ...
    $('input[type=radio][name=GeneralSession]').on('ifChecked', function (event) {
        ...
        if ($('input[type=radio][name=GeneralSession]').val() == "false") {
            ...
            $("#SubjectID").find('input[value="1"]').remove();
